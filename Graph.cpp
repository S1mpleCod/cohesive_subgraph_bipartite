...
void Graph::read_graph_adjL(){
    ...
    ifstream myfile("../livejournal_adjL_deg.txt"); //v id, dege, neighbours
    string line;
    if (myfile.is_open()){
        int edgeindex=0;
        int i=0;
        ...
        while (getline(myfile,line) ) {
            vector<int> vline; //[0] id [1] deg [2] to [??] neighbours
            string ele;
            //cout<<line<<endl;
            ...
            pstart[i+1]=pstart[i]+vline[1];
            i++;
            for(int j=2;j<vline.size();j++){
                edges[edgeindex]=vline[j];
                edgeindex++;
            }
